...
class Command(BaseCommand):
    ...
    def _create_coauthors_submission_email_template_setting(self, group: SettingGroup):
        ...
        try:
            SettingValue.objects.get(journal=None, setting=coauthor_submission_email_template)
            self.stdout.write(
                self.style.WARNING(
            ...
            self.stdout.write(
                self.style.SUCCESS("Successfully created coauthor email template setting for submission notification.")
            )
    ...
    def _create_coauthors_submission_email_subject_setting(self, group: SettingGroup):
        ...
        try:
            SettingValue.objects.get(journal=None, setting=coauthor_submission_email_subject)
            self.stdout.write(
                self.style.WARNING(
            ...
            self.stdout.write(
                self.style.SUCCESS("Successfully created coauthor email subject setting for submission notification.")
            )
